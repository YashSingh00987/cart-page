...
const Cart: React.FC = () => {
  ...
  return (
    <>
    <Flex direction={["column", "row"]} p={5} gap={5}>
      <Box flex={2}>
        <Table variant="simple" borderBottom={"2px solid #ccc"}>
          <Thead>
            <Tr>
              <Th>Product</Th>
              <Th>Price</Th>
                ...
                <Td>
                  <Flex align="center">
                    ...
                    <Button
                      ...
                      isDisabled={item.quantity >= (products.products.find(p => p.id === item.id)?.stock || 0)}
                      title={item.quantity >= (products.products.find(p => p.id === item.id)?.stock ?? 0) ? "Max stock reached" : ""}                    >
                      <IoAddSharp />
                    </Button>
                  </Flex>
                </Td>
      ...
      <Box flex={1}>
        ...
        >
          ...
          <Flex justify="space-between">
            ...
            <Text fontWeight="bold" paddingRight={"15px"}>${calculateTotal().toFixed(2)}</Text>
          </Flex>
          <Link to='/thanks'><Button colorScheme="blue" size="lg" rounded={"full"}>
            Proceed to Checkout
          </Button></Link>
        </VStack>
      </Box>
